...
def test_get_account_by_plaid_item_returns_account(db, plaid_item_factory, account_factory):
    ...
    result = get_accounts_by_plaid_item_id(db, item.profile_id, item.id)
    assert result.success
    assert result.data is not None
...
def test_create_account_balance_accepts_valid_input(db, profile_factory, account_factory, valid_create_account_balance_request_factory):
    profile = profile_factory()
    account = account_factory(profile_id=profile.id)
    with db.get_session() as session:
        old_count = session.query(AccountBalance).count()
    request = valid_create_account_balance_request_factory(account_id=account.id)
    result = create_account_balance(db, profile.id, request)
    assert result.success
    assert result.data is not None
    with db.get_session() as session:
...
def test_get_balances_by_account_obeys_start_and_end(db, account_factory, account_balance_factory):
    ...
    result = get_balances_by_account(db, account.profile_id, account.id, start, end)
    assert result.success
    assert result.data is not None
    assert len(result.data) == 1
...
def test_get_balances_by_account_gets_all_since_start_if_end_omitted(db, account_factory, account_balance_factory):
    ...
    result = get_balances_by_account(db, account.profile_id, account.id, end=end)
    assert result.success
    assert result.data is not None
    assert len(result.data) == 2
    ids = [d.id for d in result.data]
    assert balance_before_window.id in ids
    assert balance_inside_start.id in ids
    assert balance_after_start not in ids
...
def test_get_balances_by_account_gets_all_if_start_and_end_omitted(db, account_factory, account_balance_factory):
    ...
    result = get_balances_by_account(db, account.profile_id, account.id)
    assert result.success
    assert result.data is not None
...
def test_get_latest_balance_by_account_returns_latest_balance(db, account_factory, account_balance_factory):
    ...
    newer_balance = account_balance_factory(
        account_id=account.id,
        timestamp=datetime.now(tz=timezone.utc))
    result = get_latest_balance_by_account(db, account.profile_id, account.id)
    assert result.success
    assert result.data is not None
