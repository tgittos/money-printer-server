...
def fetch_historical_daily(cls, symbol: str, start: datetime = None, end: datetime = None, close_only: bool = False)\
    ...
    try:
        ...
        if ex.status == 404:
            cls.logger.warning(
                "upstream provider couldn't resolve symbol {0}".format(symbol))
...
def fetch_historical_intraday(cls, symbol: str, start: datetime = None) -> DataFrame:
    ...
    for i in range(days + 1):
        ...
        if not is_bus_day(fetch_date.date()):
            cls.logger.debug(
                "skipping historical intraday fetch for {0}, non business day".format(fetch_date))
            continue
        ...
        try:
            ...
            continue
    cls.logger.debug(
        "returning {0} datapoints after upstream walk".format(len(total_df)))
    return total_df
