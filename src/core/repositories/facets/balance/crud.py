    ...
    if request.account is not None:
        if request.start is not None:
            if request.end is not None:
                records = cls.db.query.filter(and_(
            ...
            records = cls.db.query(AccountBalance).filter(AccountBalance.account_id == request.account.id).all()
    return records
...
def get_latest_balance_by_account(self, account: Account) -> AccountBalance:
    ...
    """
    r = self.db.query(AccountBalance).filter(AccountBalance.accountId == account.id).order_by(
